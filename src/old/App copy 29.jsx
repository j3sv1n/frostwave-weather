...
function App() {
  ...
  const getUserLocation = () => {
    console.log("Attempting to get user location..."); // Debugging log
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const { latitude, longitude } = position.coords;
        console.log(`User location: Latitude ${latitude}, Longitude ${longitude}`); // Debugging log
        const locationString = `${latitude},${longitude}`;
        setCurrentLocation(locationString); // Update the current location state
        fetchWeather(locationString); // Fetch weather for the user's location
      },
      (error) => {
        console.error("Error getting user location:", error); // Log the error
  ...
  console.log(`Fetching weather for location: ${loc}`); // Debugging log
  try {
    const response = await axios.get(WEATHER_URL, {
      params: { key: API_KEY, q: loc, days: 7 },
    });
    console.log("Weather data fetched successfully:", response.data); // Debugging log
    setWeather(response.data); // Update the weather state
    setCurrentLocation(loc); // Update the current location state
    fetchAiSummary(response.data); // Fetch AI summary
    fetchMusic(response.data.current.condition.text); // Fetch music based on weather condition
  } catch (error) {
